...
other: "parameter [param] on [GET] [https://example.com/] may be vulnerable to LDAP injection, using an attack with LDAP meta-characters [|!<>=~=>=<=*(),+-\"'\\/], yielding known [activedirectory] error message [ERROR_DS_NAMING_VIOLATION], which was not present in the original response."
cwe: 90
wasc: 29
techtags: 
  - Protocol.LDAP
