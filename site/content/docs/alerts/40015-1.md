...
other: "parameter [param] on [GET] [https://example.com/] may be vulnerable to LDAP injection, using an attack with LDAP meta-characters [|!<>=~=>=<=*(),+-\"'\\/], yielding known [openldap] error message [Cannot Cancel], which was not present in the original response."
cwe: 90
wasc: 29
techtags: 
  - Protocol.LDAP
