...
other: "The query time is controllable using parameter value [test' and 0 in (select case when cast(pg_sleep(5) as varchar) > '' then 0 else 1 end ) -- ], which caused the request to take [5,000] milliseconds, when the original unmodified query with value [test] took [100] milliseconds."
cwe: 89
wasc: 19
techtags: 
  - Db.PostgreSQL
