...
other: "Cookie set in response when cookie field [JSESSIONID] is set to NULL: [valueC] Cookie set in response with borrowed (valid) cookie value in request [valueC] : [valueD] The url on which the issue was discovered was flagged as a logon page.  This issue should be manually validated by checking that the session token set on the logon page is still valid once logged in, and that the token can be used to directly access a page/resource which should require the user to be logged in."
cwe: 384
wasc: 37
alerttags: 
  - CWE-384
