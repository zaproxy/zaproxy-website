...
other: "A likely session value has appeared in URLs in the HTML output when URL parameter/pseudo URL parameter [jsessionid] is set to NULL: [valueE] When this borrowed session [valueE] is used in a subsequent request for this URL, a new session is not created. The url on which the issue was discovered was flagged as a logon page."
cwe: 384
wasc: 37
alerttags: 
  - CWE-384
